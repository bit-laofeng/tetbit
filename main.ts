...
namespace sensors {
    ...
    export function rotaryPotentiometer(pin: AnalogPin): number {
        let row2 = pins.analogReadPin(pin)
        return row2
    }
    ...
    export function piano_v2_play(): void {
        ...
        for (let j = 0; j < 8; j++) {
            ...
            DATA2 |= pins.digitalReadPin(_pianoDIO) << j
        }
        control.waitMicros(2 * 1000)
        //         serial.writeString('' + DATA + '\n');
        switch (DATA2 & 0xFF) {
    ...
    function i2cwrite1(addr: number, reg: number, value: number, value1: string) {
        ...
        let buf2 = pins.createBuffer(2 + lengths)
        //let arr = value1.split('')
        buf2[0] = reg
        buf2[1] = value
        let betys = []
        betys = stringToBytes(value1)
        for (let k = 0; k < betys.length; k++) {
            buf2[2 + k] = betys[k]
        }
    ...
    */
    ...
    export function Ultrasonic(pin: DigitalPin): number {
        ...
        // read pulse
        let e = pins.pulseIn(pin, PulseValue.High, 25000);
        let ret = e;
    ...
    function RgbDisplay(indexstart: number, indexend: number, rgb: RgbColors): void {
        for (let l = indexstart; l <= indexend; l++) {
            emRGBLight.setPixelColor(l, rgb);
        }
        emRGBLight.show();
    }

    function board_RgbDisplay(indexstart: number, indexend: number, rgb: RgbColors): void {
        for (let m = indexstart; m <= indexend; m++) {
            board_emRGBLight.setPixelColor(m, rgb);
        }
    ...
    export function rus04_rgb(pin: DigitalPin, offset: number, index: number, rgb: number, effect: number): void {
        ...
        switch (effect) {
            ...
            case ColorEffect.Breathing:
                for (let n = 0; n < 255; n += 2) {
                    emRGBLight.setBrightness(n);
                    RgbDisplay(start, end, rgb);
                    //basic.pause((255 - i)/2);
                    basic.pause((n < 50) ? 10 : (255 / n));
                }
                for (let o = 255; o > 0; o -= 2) {
                    emRGBLight.setBrightness(o);
                    RgbDisplay(start, end, rgb);
                    basic.pause((o < 50) ? 10 : (255 / o));
                }
                break;
    ...
    export function board_rus04_rgb(pin: DigitalPin, offset: number, index: number, rgb: number, effect: number): void {
        ...
        switch (effect) {
            case ColorEffect.None:
                ...
                board_RgbDisplay(start2, end2, rgb);
                break;
            case ColorEffect.Breathing:
                for (let r = 0; r < 255; r += 2) {
                    board_emRGBLight.setBrightness(r);
                    board_RgbDisplay(start2, end2, rgb);
                    //basic.pause((255 - i)/2);
                    basic.pause((r < 50) ? 10 : (255 / r));
                }
                for (let s = 255; s > 0; s -= 2) {
                    board_emRGBLight.setBrightness(s);
                    board_RgbDisplay(start2, end2, rgb);
                    basic.pause((s < 50) ? 10 : (255 / s));
                }
                break;
            case ColorEffect.Rotate:
                board_emRGBLight.setBrightness(255);
                for (let t = 0; t < 4; t++) {
                    board_emRGBLight.setPixelColor(start2, rgb);
                    board_emRGBLight.setPixelColor(start2 + 1, 0);
                    board_emRGBLight.setPixelColor(start2 + 2, 0);
                    if (index == RgbUltrasonics.All) {
                        board_emRGBLight.setPixelColor(end2 - 2, rgb);
                        board_emRGBLight.setPixelColor(end2 - 1, 0);
                        board_emRGBLight.setPixelColor(end2, 0);
                    }
                    board_emRGBLight.show();
                    basic.pause(150);
                    board_emRGBLight.setPixelColor(start2, 0);
                    board_emRGBLight.setPixelColor(start2 + 1, rgb);
                    board_emRGBLight.setPixelColor(start2 + 2, 0);
                    if (index == RgbUltrasonics.All) {
                        board_emRGBLight.setPixelColor(end2 - 2, 0);
                        board_emRGBLight.setPixelColor(end2 - 1, rgb);
                        board_emRGBLight.setPixelColor(end2, 0);
                    }
                    board_emRGBLight.show();
                    basic.pause(150);
                    board_emRGBLight.setPixelColor(start2, 0);
                    board_emRGBLight.setPixelColor(start2 + 1, 0);
                    board_emRGBLight.setPixelColor(start2 + 2, rgb);
                    if (index == RgbUltrasonics.All) {
                        board_emRGBLight.setPixelColor(end2 - 2, 0);
                        board_emRGBLight.setPixelColor(end2 - 1, 0);
                        board_emRGBLight.setPixelColor(end2, rgb);
                    }
                    board_emRGBLight.show();
                    basic.pause(150);
            ...
            case ColorEffect.Flash:
                for (let u = 0; u < 3; u++) {
                    ...
                    board_RgbDisplay(start2, end2, rgb);
                    basic.pause(100);
                    board_RgbDisplay(start2, end2, 0);
                    basic.pause(50);
                }
                break;
        }
    ...
    export function dht11value(dht11pin: DigitalPin, dht11type: DHT11Type): number {
        ...
        basic.pause(18)
        let v = pins.digitalReadPin(dht11pin)
    ...
    export function dht11value_v2(dht11pin: DigitalPin, dht11type: DHT11Type): number {
        pins.digitalWritePin(dht11pin, 0)
        basic.pause(18)
        let c = pins.digitalReadPin(dht11pin)
        pins.setPull(dht11pin, PinPullMode.PullUp);
        switch (dht11type) {
            case 0:
                let dhtvalue12 = 0;
                ...
                for (let f = 0; f <= 32 - 1; f++) {
                    ...
                    if (f > 15) {
                        if (dhtcounter12 > 10) {
                            dhtvalue12 = dhtvalue12 + (1 << (31 - f));
                        }
                    }
                }
                // serial.writeString("DHT11_V2" + "" + dhtvalue1)
                return ((dhtvalue12 & 0x0000ffff) >> 8);
                break;
    ...
    export function selectlight(selectpin: _selectlight, _status: ledon_off): void {
        ...
        pins.digitalWritePin(h, _status)
    }
    ...
    export class TM1637LEDs {
        ...
        _write_byte(b: number) {
            for (let i3 = 0; i3 < 8; i3++) {
                pins.digitalWritePin(this.dio, (b >> i3) & 1)
                pins.digitalWritePin(this.clk, 1)
                pins.digitalWritePin(this.clk, 0)
            }
            pins.digitalWritePin(this.clk, 1)
        ...
        showDP(_status: ledon_off, bit: number) {
            bit = bit % this.count
            let show2 = _status == 1 ? true : false;
            if (show2) this._dat(bit, this.buf[bit] | 0x80)
    ...
    export function Speech_recognition_get_result(): number {
        let result2 = i2cread(VOICE_IIC_ADDR, VOICE_RESULT_REG)
        return result2;
    }
